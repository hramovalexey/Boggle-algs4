/*
Extended (modified) TrieST class.
Due to the limits of the Princeton course explicit class deriving is prohibited.

Changes:
Generic value replaced by int
New method getNextNode is added
...
public class ExtTrieST {
     ...
     */
    private int get(StringBuilder key) {
    ...
    // New method
    public Node getNextNode(Node x, char newChar) {
        if (x == null) return null;
        return x.next[newChar - 65];
    }
    ...
    public Node get(Node x, StringBuilder key, int d) {
        if (x == null) return null;
    ...
    // Additional method for checking if this table includes given prefix (unlike keysWithPrefix returns boolean)
